...
오류 코드를 반환한다는 것은 **어디선가 코드를 정의한다는 것**이다.
...
- 다른 클래스에서 `Error enum` 을 `import` 해서 사용해야 하므로, `Error enum` 이 변한다면 이를 사용하는 클래스 전부를 다시 컴파일하고 다시 배치해야 한다.
  - 그래서 `Error` 클래스 변경이 어려워지고, 프로그래머는 재컴파일/재배치가 번거롭기에 새 오류 코드를 정의하고 싶지 않아지기에 새 오류 코드 추가보다 기존 오류 코드를 재사용하려 한다.

오류 코드 대신 예외를 사용하면 새 예외는 `Exception` 클래스에서 파생되기에, 재컴파일/재배치가 없이 새 예외 클래스를 추가할 수 있다.
...
**반복되는 알고리즘**은 다른 코드와 섞이면서 중복이 금방 드러나지는 않지만, 이는 분명하게 찾아 수정해야한다.

- 코드 길이가 늘어날 뿐만 아니라 알고리즘이 변경될 때, 네 부분 모두 손 봐야 한다는 단점이 있다.
...
에르허츠 데이크스트라의 구조적 프로그래밍 원칙은 **모든 함수와 함수 내 모든 블록에 입구와 출구 하나만 있어야 한다**는 것을 의미한다.

- 즉, 함수는 하나의 `return` 문 만을 가지며 `break`, `continue` 를 지양하고 `goto` 는 절대로 사용하면 안된다.

이러한 목표와 규율은 공감하지만 **함수가 작다면** 별 이점을 제공하지 못한다.

- 함수를 작게 만든다면 간혹 `return`, `break`, `continue` 을 여러 차례 사용해도 괜찮다.
  - 때로는 단일 입/출구 규칙 보다 의도를 표현하기 쉬워진다.
...
- **먼저 생각을 기록한 후, 읽기 좋게 다듬는다.** 함수도 이와 같다.
- 처음부터 탁 짜내지 않으며, 그게 가능한 사람은 없으리라.
...
- **함수**는 그 언어에서 **동사**며 **클래스**는 **명사**다.

대가(master) 프로그래머는 시스템을 구현할 프로그램이 아니라 **풀어갈 이야기라고 여긴다.**

- 시스템에서 발생하는 **모든 동작을 설명하는 함수 계층**이 바로 그 언어에 속한다.
